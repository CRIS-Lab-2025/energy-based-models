def training_epoch(s):
    for x, y in self._dataloader:
            ...
            W,S = self._energy_minimizer.compute_equilibrium(S, W, B)  # we let the network settle to equilibrium (free state)        

            # training step
            grads = self._differentiator.compute_gradient(S, W, B, target)  # compute the parameter gradients
