...
class EnergyBasedModel(nn.Module):
    ...
    def init_weights(self):
        ...
        self.W[self.idx_in[:, None], self.idx_h] = W_ih
        self.W[self.idx_h[:, None], self.idx_in] = W_ih.T
        self.W[self.idx_h[:, None], self.idx_out] = W_ho
        self.W[self.idx_out[:, None], self.idx_h] = W_ho.T
    ...
    def compute_energy(self, s, u):
        ...
        term3 = -0.5 * torch.sum(rho_u @ self.W * rho_u, dim=1)
        return term1 + term2 + term3

    def energy_grad(self, s, rho_u, rho_prime, y_target=None, h_target=None):
        full_term = rho_u @ self.W + self.b
    ...
    def train(self, cutoff_accuracy = 1.0):
        ...
        W_ih = W[idx_in[:, None], idx_h].clone()
        W_ho = W[idx_h[:, None], idx_out].clone()
        ...
        for epoch in range(self.epochs):
            ...
            # Update
            W[idx_in[:, None], idx_h] += self.lr_ih * dW_ih / (self.subset_size // self.batch_size)
            W[idx_h[:, None], idx_in] += self.lr_ih * dW_ih.T / (self.subset_size // self.batch_size)
            W[idx_h[:, None], idx_out] += self.lr_ho * dW_ho / (self.subset_size // self.batch_size)
            W[idx_out[:, None], idx_h] += self.lr_ho * dW_ho.T / (self.subset_size // self.batch_size)
