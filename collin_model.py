...
class CollinNetwork:
    ...
    def energy(self, layers):
        ...
        if energy_fn == 'none':
            ...
            linear_terms = -sum([torch.matmul(self.activation(layer), b) for layer, b in zip(layers, self.biases)])
            # quadratic_terms: for each adjacent pair of layers.
            quadratic_terms = -sum([
                ((torch.matmul(self.activation(pre), W)) * self.activation(post)).sum(dim=1)
    ...
    def cost(self, layers):
        ...
        return 0.5*((layers[-1] - self.y_data_one_hot) ** 2).sum(dim=1)
    ...
    def activation_prime(self, neurons):

        activation = self.hyperparameters["activation"] if "activation" in self.hyperparameters else "pi"

        return get_activation_prime(activation, neurons)
    ...
    def negative_phase(self, n_iterations):
        ...
        for _ in range(n_iterations):
            ...
            for k in iter_order:
                hidden_input = self.activation_prime(current_layers[k])*(torch.matmul(new_layers[k-1], self.weights[k - 1]) +
                                torch.matmul(current_layers[k + 1], self.weights[k].t()) +
                                self.biases[k])
                new_layers[k]=hidden_input
            # Compute output layer.
            output_input = self.activation_prime(current_layers[-1])*(torch.matmul(new_layers[-2], self.weights[-1]) + self.biases[-1])
            new_layers[-1] = output_input
    ...
    def positive_phase(self, n_iterations, beta):
        ...
        for _ in range(n_iterations):
            ...
            for k in range(len(self.layers) - 2, 0, -1):
                back_input = self.layers[k] - self.activation_prime(self.layers[k])*(torch.matmul(self.layers[k - 1], self.weights[k - 1]) +
