...
def train_ebm(model, train_loader, val_loader, test_loader=None, epochs=50, 
              scheduler_type='plateau', patience=10, early_stopping=True):
    """
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target_onehot) in enumerate(tqdm(train_loader, desc=f'Epoch {epoch+1}')):
            ...
            #target_idx = torch.argmax(target_onehot, dim=1).detach().cpu().numpy()
            train_targets.extend(target_onehot)
        ...
        with torch.no_grad():
            for data, target_onehot in val_loader:
                ...
                #target_idx = torch.argmax(target_onehot, dim=1).cpu().numpy()
                val_targets.extend(target_onehot)
    ...
    if test_loader is not None:
        ...
        with torch.no_grad():
            for data, target_onehot in test_loader:
                ...
                target_idx = torch.argmax(target_onehot, dim=1).cpu().numpy()
                test_targets.extend(target_idx)
...
def run_training_experiment(device='cuda' if torch.cuda.is_available() else 'cpu'):
    """
    ...
    """
    # Generate dataset
    X, y_onehot, y_idx = generate_circles_dataset(n_samples=1000, noise=0.1)
    ...
    model_config = {
        ...
        'hidden_sizes': [64, 2],  # Output size 2 for binary classification
        'beta': 0.1,
        'dt': 0.1,
        'n_steps': 20
    }
...
def run_mnist_experiment(device='cuda' if torch.cuda.is_available() else 'cpu'):
    """
    ...
    # train_dataset.targets = torch.nn.functional.one_hot(train_dataset.targets, num_classes=10).float()
    test_dataset.data = test_dataset.data.view(-1, 28*28).float() / 255.0
    # test_dataset.targets = torch.nn.functional.one_hot(test_dataset.targets, num_classes=10).float()
    ...
    model_config = {
        ...
        'hidden_sizes': [512, 10],  # Output size 10 for 10 digits
        'beta': 0.1,
        'dt': 0.1,
        'n_steps': 20
    }
...
if __name__ == "__main__":
    ...
    model, history = run_training_experiment(device=device)
    #model, history = run_mnist_experiment(device=device)
